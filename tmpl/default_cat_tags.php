    ...
    if (is_array($category)) { // check category contains items
        foreach ($category as $item) {
            ...
            if ($iso_entree == "webLinks") {
                ...
                $arr_css = array("title" => $title, "cat" => $iso->cats_lib[$item->catid],"date" => $libcreated.date($libdateformat, strtotime($item->created)), "visit" => $item->hits, "intro" => $item->description,"tagsimg" => $tag_img,"catsimg" => $cat_img, "tags" => $itemtags);
                ...
                if ($params->get('applyplugins','true') == 'true') { // apply content plugins on weblinks
